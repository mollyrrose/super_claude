...
source: hermes-agent-converted
upstream_path: software-development/hermes-s6-container-supervision/SKILL.md
---

> _Converted from Hermes Agent. Some tool references in the body may name Hermes-specific modules (e.g. `tools.file_operations`); the Claude Code equivalents are typically `Read`, `Edit`, `Write`, `Bash`, `Glob`, `Grep`. Treat them as guidance, not literal API calls._
