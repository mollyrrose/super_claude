...
STATE_FILE = CLAUDE_DIR / ".qrev_auto_state.json"
# Per-session file-list. /qRev reads this to scope to "files THIS window
# edited", not "all uncommitted changes" (which may include another window).
SESSION_FILES_DIR = CLAUDE_DIR / ".qrev_session_files"
STALE_DAYS = 30
...
def main() -> int:
    ...
    # Append the edited file path to ~/.claude/.qrev_session_files/<sid>.txt
    # so /qRev can scope to "files THIS window touched", not "everything
    # uncommitted on disk" (which can include another window's work on a
    # shared working tree). Best-effort; never block the hook.
    file_path = (tool_input.get("file_path") or "").strip()
    if file_path:
        try:
            SESSION_FILES_DIR.mkdir(parents=True, exist_ok=True)
            log_path = SESSION_FILES_DIR / f"{session_id}.txt"
            # Read existing lines to de-dupe (file lists are short; cheap).
            ...
            if log_path.exists():
                try:
                    ...
                    existing = set()
            norm = str(Path(file_path).resolve()) if Path(file_path).is_absolute() else file_path
            if norm not in existing:
                with open(log_path, "a", encoding="utf-8") as f:
                    f.write(norm + "\n")
        except Exception:
            pass  # never block on session-file logging
