...
def test_run_codex_challenge_skip_on_missing_binary():
    """Test run_codex_challenge returns skip when binary missing."""
    with patch("shutil.which", return_value=None):
        result = run_codex_challenge(["test.txt"], "main")
        assert not result["success"]
        assert result["skip_reason"] == "codex binary not found"
        assert "CODEX_CLI_MISSING" in result["error"]
    print("[ok] run_codex_challenge: skips gracefully when binary missing")


def test_run_codex_challenge_skip_on_missing_auth():
    """Test run_codex_challenge returns skip when auth missing."""
    with patch("shutil.which", return_value="/fake/codex"):
        with patch.dict(os.environ, {"CODEX_HOME": "/nonexistent"}, clear=False):
            ...
            result = run_codex_challenge(["test.txt"], "main")
            assert not result["success"]
            assert result["skip_reason"] == "codex auth missing"
            assert "CODEX_AUTH_FAILED" in result["error"]
    print("[ok] run_codex_challenge: skips gracefully when auth missing")
