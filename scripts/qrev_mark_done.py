...
def main() -> int:
    ...
    # Truncate the per-session file-list so the NEXT /qRev (or auto-qRev)
    # only sees files this session edits AFTER this reset. Best-effort.
    try:
        log_path = SESSION_FILES_DIR / f"{sid}.txt"
        if log_path.exists():
