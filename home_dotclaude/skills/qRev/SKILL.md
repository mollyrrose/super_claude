...
- **`mp-code-review` (Pass 1)** -- two-axis Standards+Spec review using Matt Pocock's methodology (`~/.claude/skills/mp-code-review/SKILL.md`). Runs as an additional Pass 1 agent. Spawns two internal parallel sub-agents (Standards axis + Spec axis) and reports findings separately. Provides a second opinion on standards compliance and spec adherence alongside the main quality agents.

- **`mp-diagnosing-bugs` (Pass 3)** -- architecture-focused debuggability review (`~/.claude/skills/mp-diagnosing-bugs/SKILL.md`). Runs as an additional Pass 3 agent. Reviews the changed code for: (a) code that will be hard to reproduce when it fails (tight feedback loop difficulty), (b) missing test seams that prevent regression tests from being written at the correct boundary, and (c) hidden coupling that would make bisecting the bug difficult. Surfaces architectural debts that prevent good debugging -- flags them for `/improve-codebase-architecture`.
...
### Fleet-integrity enforcement (no partial runs — HARD protocol)

Partial fleets have actually been dispatched in practice (observed 2026-08-07: a
concurrent window ran "representative"-subset qRevs). The full-roster rule is
therefore a PROTOCOL with checkable steps, not a reminder:

1. **Manifest BEFORE dispatch.** Before launching Phase B, write the complete
   roster manifest — every agent of every pass, one line:
   `fleet: [pass1: <a,b,...>, pass2: <...>, pass3: <...>]` — and COUNT it
   against the roster definition (`/rev`'s "`/rev exhaustive` (3-pass)" section
   plus the permanent members above). Dispatching from a manifest shorter than
   the roster is forbidden; fix the manifest first.
2. **Attest in the report.** The Answer-First line's `Agents: <N>/<M> engaged`
   is an attestation: `<M>` MUST equal the manifest count and `<N>` the agents
   that actually engaged scope. If N < M after the one re-dispatch retry, every
   missing agent is listed BY NAME under "Coverage gaps".
3. **A partial dispatch is an INVALID run, not a smaller run.** If at any point
   a pass turns out to have been launched with fewer agents than its roster, do
   not synthesize and do not report — re-dispatch the missing agents, wait for
   them, then synthesize from the complete fleet. Emitting a report from a
   knowingly partial fleet violates this skill.
4. **No partial mode exists for ANY caller.** Auto-qRev, `topic:`, `<path>`,
   `PR#`, `branch` — scope may narrow, the FLEET never does. A "quick qRev",
   "mini qRev", or "qRev with just the security agents" does not exist. If the
   user asks for one, say so and run `/rev` (single-pass) or `/qMin` instead —
   never a reduced thing labelled qRev.
...
## Durable report file + learning feed (always-on — nothing is ever lost)

Every `/qRev` run (including auto-qRev) persists its results to disk the moment
the synthesis exists — BEFORE printing the report to the terminal — so a crash,
a context compaction, or a swallowed tool result can never lose a finished
review:

1. **Full report file.** Write the complete fused report (Answer-First line,
   P0-P3 punch-list, per-agent verdicts, heatmap, coverage gaps, fleet manifest
   + attestation) to `<repo>/.claude/review-log/qrev-reports/<YYYYMMDD-HHMMSS>_<sessid6>.md`.
   Create the directory if missing. `.claude/review-log/` follows the existing
   convention: worktree-local, gitignored state — durable on disk, not committed.
2. **Findings rows for the learning loop.** Append every synthesis finding as
   one JSONL row to `<repo>/.claude/review-log/findings.jsonl` with
   `source: "qrev-fleet"` (same shape as the `semgrep` / `rev-standards` rows:
   file, line, severity P0-P3, rule/lens attribution, one-line summary, plus the
   report filename). This plugs the fleet's output into the EXISTING `/rev-learn`
   self-learning loop — it already consumes `findings.jsonl`, so accepted fixes
   get clustered and patterns seen >=3 times get promoted to candidate Semgrep
   rules / CODING_STANDARDS notes per rev-learn's own rules. No new machinery.
3. **Fix outcomes back into the file.** After the auto-fix phase, append the
   `- fix ...` / `- skip ...` status lines to the SAME report file, so the file
   records not just what was found but what was done about it.
4. **Memory supplement (conservative bar).** After the run, if the review
   surfaced an EVERGREEN pattern — the same class of finding recurring across
   sessions (check the previous files in `qrev-reports/`), or a project-level
   constraint every future session must know — write/update ONE memory entry in
   the current project's memory dir per the global memory rules (check for an
   existing entry first, update rather than duplicate, add the MEMORY.md router
   line). One-off findings do NOT go to memory — the report file +
   `findings.jsonl` already hold them; memory is only for what passes the
   "still matters in a year" test.

If writing any of these files fails (permissions, odd repo state), say so in
one line and continue — persistence must never block the review itself.
...
- **Durable copy first**: by the time the report prints, it is already on disk at `.claude/review-log/qrev-reports/<stamp>_<sessid6>.md` and its findings appended to `findings.jsonl` (see "Durable report file + learning feed"). The terminal print is a copy, never the only artifact; the report ends with the file path so the user can find it later.
